...
class GraderApp:
    def __init__(self, root):
        ...
        self.label_json.grid(row=2, column=0, padx=10, pady=10)
        self.entry_json = tk.Entry(root, width=40)
        self.entry_json.grid(row=2, column=1, padx=10, pady=10)
        self.button_browse_json = tk.Button(root, text="Browse", command=self.browse_json)
        self.button_browse_json.grid(row=2, column=2, padx=10, pady=10)

        # Verbose mode checkbox
        self.verbose_var = tk.BooleanVar()
        self.checkbox_verbose = tk.Checkbutton(root, text="Verbose Mode", variable=self.verbose_var)
        self.checkbox_verbose.grid(row=3, column=0, padx=10, pady=10, columnspan=2)

        # Debug mode checkbox
        self.debug_var = tk.BooleanVar()
        self.checkbox_debug = tk.Checkbutton(root, text="Debug Mode", variable=self.debug_var)
